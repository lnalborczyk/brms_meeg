...
To overcome the limitations of averaging, a number of model-based options for estimating ERPs have been proposed. These approaches are justified by the observation that the averaging approach to ERPs is equivalent to an intercept-only linear regression in a simple categorical design with no overlapping factors [@smith2014a]. The regression approach to ERPs is much more flexible than simpl averaging, as it allows modelling linear and non-linear effects of continuous predictors such as word frequency or age [e.g., @smith2014a; @smith2014b; @tremblay2014] and allow modelling and disentangling overlapping processes [e.g., @skukies_brain_2024; @skukies_modelling_2021] as implemented in the UNFOLD toolbox [@ehinger_unfold_2019]. The regression approach, as implemented for instance in the LIMO toolbox [@pernet2011], is a multi-stage procedure in which a participant-level regression model is fitted independently to each datapoint (e.g., channel, timestep, etc.) to estimate the ERP, followed by group-level statistical analyses of the regression coefficients, itself followed by multiplicity corrections or cluster-based inference [for recent applied examples, see for instance @dunagan2025; @wüllhorst2025]. Whereas this approach allows including linear and non-linear discrete and continuous predictors when estimating the ERP, it has several limitations. First, the participant-level modelling ignores spatiotemporal dependencies as it treats all datapoints independently. Second, this approach does not take into account the participant (or stimulus) variability when testing group-level hypotheses (as could be done for instance via multilevel modelling). Finally, because the output of this procedure is summarised by cluster-based inference, its conclusions suffer from the same limitations discussed in the previous section.

-->

To overcome the limitations of simple averaging, several model-based approaches for estimating ERPs have been proposed. These methods are motivated by the observation that traditional ERP averaging is mathematically equivalent to fitting an intercept-only linear regression model in a simple categorical design without overlapping events [@smith2014a]. In contrast to simple averaging, regression-based approaches to ERP estimation offer substantially greater flexibility. Notably, they allow for the modelling of both linear and nonlinear effects of continuous predictors--such as word frequency or age [e.g., @smith2014a; @smith2014b; @tremblay2014]--and enable the disentangling of overlapping cognitive processes [e.g., @ehinger_unfold_2019; @skukies_brain_2024; @skukies_modelling_2021]. One widely used implementation of this approach is provided by the LIMO EEG toolbox [@pernet2011], which follows a multi-stage analysis pipeline. First, a separate regression model is fit for each datapoint (e.g., each time point and electrode) at the individual participant level to estimate ERP responses. This is followed by group-level statistical analyses of the resulting regression coefficients, often accompanied by multiple comparison corrections or cluster-based inference [for recent applied examples, see @dunagan2025; @wüllhorst2025].

Although this framework allows for the inclusion of a wide range of predictors--both continuous and categorical, linear and nonlinear--it still has important limitations. First, fitting separate models for each datapoint ignores the spatiotemporal dependencies inherent in M/EEG data, potentially reducing statistical power and interpretability. Second, the subsequent group-level analyses typically do not account for hierarchical dependencies, between-participant or between-stimulus variability, which could otherwise be addressed through multilevel modelling. Finally, because the output of this procedure is summarised by cluster-based inference, its conclusions remain subject to the limitations discussed in the previous section.

<!--

Building upon the regression framework, recent advances allow modelling and disentangling overlapping processes [e.g., @skukies_brain_2024; @skukies_modelling_2021] as implemented in the UNFOLD toolbox [@ehinger_unfold_2019]...

In addition to modelling the non-linear effects of continuous predictors (e.g., word frequency) on the ERP amplitude, GAMs have been used for modelling the effect of time, that is, the shape of the ERP itself [@abugaber2023; @baayen2018; @meulman2015; @meulman2023]... In the next section, we turn to a brief introduction to generalised additive models and discuss their use for modelling M/EEG timeseries...

-->

Beyond modelling nonlinear effects of continuous predictors on ERP amplitudes, generalised additive models (GAMs) have been employed to capture the temporal dynamics of ERPs themselves, effectively modelling the shape of the waveform over time [@abugaber2023; @baayen2018; @meulman2015; @meulman2023]. This approach allows for the estimation of smooth, data-driven functions that characterise how neural responses evolve over time, offering a flexible alternative to traditional linear models. In the following section, we provide a brief introduction to GAMs, highlighting their applicability to M/EEG time series analysis and the advantages they offer over conventional methods.

## Generalised additive models

In generalised additive models (GAMs), the functional relationship between the predictors and the response variable is decomposed into a sum of low-dimensional non-parametric functions. A typical GAM has the following form:
...
where $\beta_{jk}$ is the weight (coefficient) associated with the $k$th basis function $b_{jk}()$ evaluated at the covariate value $x_{ij}$ for the $j$th smooth function $f_{j}$. To clarify the terminology at this point: *splines* are functions composed of simpler functions. These simpler functions are called *basis functions* (e.g., cubic polynomial, thin-plate) and the set of basis functions is called a *basis*. Each basis function gets its coefficient and the resultant spline is the sum of these weighted basis functions (@fig-intro-gam\ignorespaces A). Splines coefficients are penalised (usually through the squared of the smooth functions' second derivative) in a way that can be interpreted, in Bayesian terms, as a prior on the "wiggliness" of the function [@miller2025; @wood2017]. In other words, more complex (wiggly) basis functions are automatically penalised.
...
```{r fig-intro-gam, echo = FALSE, fig.width = 9, fig.cap = "Different types of GAMs. **A**: GAMs predictions are computed as the weigthed sum (in black) of basis functions (here thin-plate basis functions, in grey). **B**: Constant-effect GAM, with 5 participants in colours and the group-level prediction in black. **C**: Varying-intercept + varying-slope GAMM (with constant smoother). **D**: Varying-intercept + varying-slope + varying-smoother GAMM. In this model, each participant gets its own intercept, slope, and degree of 'wiggliness' (smoother)."}
########################
...
In the following, we introduce our proposed approach based on Bayesian generalized additive multilevel models (Bayesian GAMMs), which builds on these strengths while providing fully probabilistic estimates of ERP dynamics. We illustrate how this framework enables direct inference on the presence and timing of neural effects at the trial level, across conditions and participants, and show that it outperforms conventional methods in both simulated and real M/EEG datasets.
...
We then compute the posterior probability of the slope for `condition` being above $0$ (@fig-post-prob-ratio, left). From this quantity, we compute the ratio of posterior probabilities (i.e., $p/(1-p)$) and visualise the timecourse of this ratio superimposed with the conventional thresholds on evidence ratios (@fig-post-prob-ratio, right). Note that a ratio of 10 means that the probability of the difference being above 0 is 10 times higher than the probability of the difference not being above 0, given the data, the priors, and other model's assumptions.[^1] Thresholding the posterior probability ratio thus provides a model-based approach for estimating the onset and offset of M/EEG effects, whose properties will be assessed in the simulation study. An important advantage is that the proposed approach can be extended to virtually any model structure.

[^1]: This posterior probability ratio, or posterior odds, is equivalent to a Bayes factor, assuming 1:1 prior odds.
...
Note: Given the focus on the precision of onset/offset estimates, is it correct/relevant/sensible to mention that we are interested here in quantifying type-S and type-M (rather than type-1 or type-2) errors?

-->

We then assessed the performance of the proposed approach by computing the difference between the true and estimated onset/offset of the EEG difference according to various `k` (basis dimension) and `threshold` values. Remember that the EEG signal was generated from a truncated Gaussian with an objective onset at 160 ms, a maximum at 250 ms, and an offset at 342 ms. @fig-onset-error shows that the multilevel GAM can almost exactly recover the true onset and offset values, given some reasonable choice of `k` and `threshold` values. We provide more detailed recommendations on how to set `k` in @sec-basis. This figure further reveals that the optimal `k` and `threshold` values may differ for the onset and offset values, and there there seems to exist a trade-off between these two parameters: lower `k` values lead to poorer estimations, but these poor estimations can be compensated (only to some extent) by higher `threshold` values (and reciprocally).
...
## Comparing the onsets/offsets estimates across approaches
...
To assess the accuracy of group-level onset estimation, the various methods were compared using the bias (defined as the median difference between the estimated and true value of the onset/offset), median absolute error (MAE), root mean square error (RMSE), variance, and median absolute deviation (MAD) of onset/offset estimates computed on 10,000 simulated datasets. As in @rousselet_using_2025, each participant was assigned a random onset between 150 and 170ms. Whereas the focus of the present article focuses is on one-dimensional signals (e.g., one M/EEG channel), we provide a 2D application in @sec-2D.

## Application to actual MEG data

To complement the simulation study, we evaluated the performance of all methods on actual MEG data [decoding results from @nalborczyk:inprep]. In this study, we conducted time-resolved multivariate pattern analysis (MVPA, also known as decoding) of MEG data during reading tasks. As a result, we obtain a timecourse of decoding performance (ROC AUC), bounded between 0 and 1, for each participant ($N=32$). Next, we wanted to *test* whether the group-level average decoding accuracy is above chance (i.e., 0.5) at each timestep (@fig-decoding-data). To achieve this, we fitted a BGAM as introduced previously, but we replaced the $\mathrm{Normal}$ likelihood function by a $\mathrm{Beta}$ one to account for the bounded nature of AUC values (between 0 and 1) [for a tutorial on Beta regression, see @coretta2025].
...
meg_decoding_gam <- brm(
    ...
    # auc_mean ~ 1 + baseline + s(time, bs = "tp", k = 30, by = baseline),
    ...
    # file = "models/meg_decoding_gam_baseline.rds"
    )
...
prob_y_above_0 <- data.frame(time = unique(meg_decoding_gam$data$time) ) %>%
    ...
    mutate(prob_ratio = m / (1 - m) ) %>%
    # ensuring there is no 0 or +Inf values
    mutate(prob_ratio = pmin(prob_ratio, ndraws(meg_decoding_gam) ) ) %>%
    mutate(prob_ratio = pmax(prob_ratio, 1 / ndraws(meg_decoding_gam) ) ) %>%
    ungroup()
...
# read and bind all files, adding a sim_id
...
#     summarise(
#         bias_onset = median(onset - true_onset),
#         MAE_onset = median(abs(onset - true_onset) ),
#         variance_onset = var(onset),
#         bias_offset = median(offset - true_offset),
...
Results are further summarised in @tbl-simulation-results, which shows that the `BGAM` is almost perfectly unbiased (i.e., it has a bias of approximately 2ms for the onset and 4ms for the offset). The `Bias` column shows that all methods tend to estimate the onset later than the true onset and to estimate the offset earlier than the true offset. As can be seen from this table, the `BGAM` has the best performance on all included metrics.

```{r tbl-simulation-results, eval = TRUE, echo = FALSE, results = "asis"}
#| tbl-cap: "Summary statistics of the onset and offset estimates for each method (in ms, ordered by the MAE) according to the main simulation study."
...
# reading and binding all files, adding a sim_id
...
We kept the exemplary models simple, but can be extended by adding varying/random effects (intercept and slope) for item (e.g., word)... but also continuous predictors at the trial level, overlapping factors, etc...
...
```{r fig-timegen-tickertape, eval = FALSE, echo = FALSE, dpi = 300, out.width = "75%", fig.cap = "Exemplary group-level average cross-temporal generalisation matrix of decoding performance (ROC AUC)."}
...
# data <- np$load("data/group_level_decoding_7l_2s_vs_7l_3s_visual_blocks_temporal_generalisation_all.npy")
...
where $\left\|\mathbf{s}_i-\mathbf{k}_{m}\right\|$ is the Euclidean distance between the covariate $\mathbf{s}_{i}$ and the knot location $\mathbf{k}_{m}$. We fitted this model using `brms`... using the `t2()` smooth constructor with full penalties [@pedersen_hierarchical_2019]...

```{r gam-timegen, eval = FALSE, echo = TRUE, results = "hide"}
...
# fitting a GAM with two temporal dimensions to empirical data
timegen_gam <- brm(
    # 2D thin-plate spline (tp)
    # auc ~ t2(train_time, test_time, bs = "tp", k = 30),
    auc ~ t2(train_time, test_time, bs = "tp", k = 20, full = TRUE),
    data = timegen_data,
    family = Beta(),
    warmup = 1000,
    iter = 3000,
    chains = 8,
    cores = 8,
    # file = "models/timegen_gam_meg_t2_k30.rds"
    file = "models/timegen_gam_meg_t2_k20_full.rds"
    )

# retrieving the data
# timegen_data <- timegen_gam$data
...
```{r gam-timegen-post-preds, eval = FALSE, echo = FALSE, cache = FALSE, fig.width = 9, fig.asp = 0.5, fig.cap = "Simulated data with thresholds derived from the BGAM (left), predicted AUC values (middle), and posterior probabily ratios of decoding accuracy being above chance level (right) according to the bivariate BGAM (for k=10, top row, and k=20, bottom row)."}
# defining a function to plot posterior predictions
...
baseline_sd <- timegen_data %>%
    filter(train_time < 0 & test_time < 0.2) %>%
    summarise(sd(auc) ) %>% pull()
...
Here we should provide recommendation about how to define `k`... An option is to vary `k` and examine the predictions and posterior predictive checks (PPCs) of each model... In this example (@fig-choose-k)... However, it is not possible to provide general recommendations, as the optimal `k` depends on the sampling rate, the preprocessing steps (e.g., signal-to-noise ratio, low-pass filtering, etc), and the neural dynamics of the phenomenon under study. Choose `k` as large a possible (given limits on computation time etc)? Or use LOOIC/WAIC (seem to select too simple models)?
