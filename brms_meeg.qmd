...
floatsintext: true
numbered-lines: true
bibliography: meeg_modelling.bib
...
# theme_set(theme_light(base_size = 10, base_family = "CMU Sans Serif") )
...
The smoothness and general behaviour of the function $f$ depends only on the choice of covariance kernel, see <https://mc-stan.org/docs/functions-reference/matrix_operations.html#gaussian-process-covariance-functions>. It ensures that values that are close together in the input space will be mapped to similar output values... From this perspective, $f$ is a realisation of an infinite dimensional normal distribution:

$$
f \sim \mathrm{Normal}(0, C(\lambda))
$$

where $C$ is a covariance kernel with hyperparameters $\lambda$ that defines the covariance between two function values $f\left(t_1\right)$ and $f\left(t_2\right)$ for two time points $t_1$ and $t_2$ [@rasmussen2005]. Similar to the different choices of the basis function for splines, different choices of the covariance kernel lead to different GPs. In this article, we consider the squared-exponential (a.k.a. Gaussian or radial basis function) kernel, which computes the squared distance between points and converts it into a measure of similarity. It is defined as:
...
Following the approach used by @sassenhagen2019 and @rousselet_using_2025, we simulated EEG data stemming from two conditions, one with noise only, and the other with noise + signal. As in previous studies, the noise was generated by superimposing 50 sinusoids at different frequencies, following an EEG-like spectrum [see details and code in @yeung2004]...

For simulating EEG data, see <https://github.com/GRousselet/onsetsim> and <https://github.com/unfoldtoolbox/UnfoldSim.jl>...

Copy-pasted from @rousselet_using_2025: "The signal was a truncated Gaussian defining an objective onset at 160 ms, a maximum at 250 ms, and an offset at 342 ms. It was created by getting the probability density function of a standard normal with 93 points from x= 1.5 to 1.5, rescaled in amplitude between 0 and 1 and padded with 79 zeros to the left and to the right, to form a time series of 251 points, from 0 to 500 ms, in steps of 2 ms (500 Hz sampling rate)."
...
Models were fitted using the `brms` package [@burkner_brms_2017; @burkner_advanced_2018; @nalborczyk2019]...
...
gam_ratio_plot <- prob_y_above_0 %>%
    ...
    labs(
        x = "Time (ms)",
        y = expression(Pr(beta>0~"|"~data, model) / (1 - Pr(beta>0~"|"~data, model) ) )
        )
...
Now, we want to *test* whether the group-level average decoding accuracy is above chance (i.e., 0.5) at each timestep. We use a similar GAM/GP as previously, but we replace the $\mathrm{Normal}$ likelihood function by a $\mathrm{Beta}$ one.
...
plot_post_decoding_ratio <- function (model, data, chance_level = 0.5, eps = 0.1) {
    ...
    prob_y_above_0 %>%
        ...
        labs(
            x = "Time (s)",
            y = expression(Pr(AUC>chance~"|"~data, model) / (1 - Pr(AUC>chance~"|"~data, model) ) )
            )
...
```{r gp-timegen, eval = FALSE, message = FALSE}
...
Comparing performance with uncorrected massive test, classical correction procedures (e.g., Bonferroni, FDR, FWER), cluster-based permutation test, cluster-depth, change point, [TFCE](https://jaromilfrossard.github.io/permuco4brain/articles/tfce.html)... We used the `permuco` package [@frossard2021]...

```{r permutations-1d, echo = FALSE, fig.cap = "T-values timecourse with onset identified using the permuco package (clustermass method)."}
...
```{r changepoint, echo = FALSE, fig.cap = "T-values timecourse with true onset and onset identified using the changepoint package (binary segmentation method)."}
# computing the timecourse of t-values^2
t_values <- raw_df %>%
    summarise(y = mean(y), .by = c(x, condition, participant) ) %>%
    pivot_wider(names_from = condition, values_from = y) %>%
    # head()
    group_by(x) %>%
    summarise(tvalues = t.test(cond1, cond2)$statistic^2 ) %>%
    ungroup()

# using the changepoint package to identify onsets and offsets
res <- cpt.meanvar(data = t_values$tvalues, method = "BinSeg", Q = 2)

# plotting the results
true_onset <- 160
t_values %>%
    ggplot(aes(x = x, y = tvalues) ) +
    geom_line(linewidth = 1) +
    geom_vline(xintercept = true_onset) +
    geom_vline(xintercept = Xf[res@cpts[1]], linetype = "dotted") +
    labs(x = "Time (ms)", y = bquote(t^2) ) +
    # theme_light() +
    ggtitle(paste0("Change point onset: ", Xf[res@cpts[1]], "ms, true onset: ", true_onset, "ms") )
```

Now cluster-based permutations in 2D...

```{r permutations-2d, fig.cap = "Blah blah..."}
...
# or using the MNE package in Python
import mne
```
...
TO-DO: Prepare a wrapper `R` package and show how to call it in `Python` and integrate it with `MNE-Python` [@gramfort2013] pipelines...
...
Where... Then, we can compute p-values for each timestep.... we permute conditions, compute the TFCE score for the permuted set, and take the max(TFCE)... But see @sassenhagen2019...
